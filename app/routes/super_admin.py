...
def broadcast_notification():
    ...
    if image_file and image_file.filename:
        ...
        StorageService.upload_file(image_file, unique_filename, folder='notifications')
...
@super_admin_bp.route('/notification_image/<path:filename>')
def download_notification_image(filename):
    import os
    basename = os.path.basename(filename)
    from app.services.storage_service import StorageService
    url = StorageService.get_file_url(basename, 'notifications')
    if url:
        return redirect(url)
